    ...
    private function pointUpload($path, $stream, $params)
    {
        ...
        if (is_array($params)) {
            foreach ($params as $key => $val) {
                $headers['X-Upayun-Meta-' . $key] = $val;
            }
        }
        $res = $req->request('PUT', $path)
            ->withHeaders(array_merge(array(
                'X-Upayun-Multi-Stage' => 'initiate',
                'X-Upayun-Multi-Type' => Psr7\mimetype_from_filename($path),
                'X-Upayun-Multi-Length' => $stream->getSize(),
            ), $headers))
            ->send();
        ...
        do {
            ...
            $res = $req->request('PUT', $path)
                ->withHeaders(array(
                    'X-Upayun-Multi-Stage' => 'upload',
                    'X-Upayun-Multi-Uuid' => $uuid,
                    'X-Upayun-Part-Id' => $partId
                ))
