 ...
 * 又拍云云存储、云处理接口
 *
 * Upayun 类实现了又拍云云存储和云处理的所有接口，通过该类可以实现文件上传、下载；图片视频等多媒体资源云处理。
...
class Upyun
{
     ...
     * - 添加[文件元信息](http://docs.upyun.com/api/rest_api/#metadata) `write($path, $content, array('X-Upayun-Meta-Foo' =>
     * 'bar'))`
     ...
     *    ),
     *    ... // 同时还可以添加其他任务
     * ), Upayun::$PROCESS_TYPE_MEDIA, $source)
     * ```
     *
     * @param array $tasks 需要处理的任务
     * @param string $type 异步云处理任务类型，可选值:
     * - `Upayun::$PROCESS_TYPE_MEDIA` 异步音视频处理
     * - `Upayun::$PROCESS_TYPE_ZIP` 文件压缩
     * - `Upayun::$PROCESS_TYPE_UNZIP` 文件解压
     * - `Upayun::$PROCESS_TYPE_SYNC_FILE` 文件拉取
     * - `Upayun::$PROCESS_TYPE_STITCH` 图片拼接
