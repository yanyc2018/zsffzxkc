 ...
 * 3，一个用户可以属于多个用户组(jy_auth_group_access表 定义了用户所属用户组)。我们需要设置每个用户组拥有哪些规则(jy_auth_group 定义了用户组权限)
 * 
 * 4，支持规则表达式。
 *      在jy_auth_rule 表中定义一条规则时，如果type为1， condition字段就可以定义规则表达式。 如定义{score}>5  and {score}<100  表示用户的分数在5-100之间时这条规则才会通过。
 */
namespace com;
...
class Auth{
    ...
    protected $_config = array(
        ...
        'auth_group'        => 'jy_auth_group',        // 用户组数据表名
        'auth_group_access' => 'jy_auth_group_access', // 用户-用户组关系表
        'auth_rule'         => 'jy_auth_rule',         // 权限规则表
        'auth_user'         => 'jy_admin'             // 用户信息表
    );
